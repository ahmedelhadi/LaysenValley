...
<div class="mb-2">
    ...
    {!! Form::text('title['.App::getLocale().']', $block->title[App::getLocale()] ?? '', ['required', 'class' => 'form-control', 'autocomplete' => 'off', 'id' => 'title_ar']) !!}

</div>

<div class="mb-2">
    {!! Form::label('sub_title-ar', trans('admin.title') , ['class' => 'form-label'] ) !!}
    {!! Form::text('sub_title['.App::getLocale().']', $block->sub_title[App::getLocale()] ?? '', ['required', 'class' => 'form-control', 'autocomplete' => 'off', 'id' => 'title_ar']) !!}
</div>
...
<div class="mb-2">
    ...
    {!! Form::textarea('desc['.App::getLocale().']', $block->desc[App::getLocale()]  ?? '' , [ 'class' => 'textarea form-control ckeditor', 'id' => 'ckeditor', 'placeholder' => trans('admin.desc'), 'rows' => 5 ]) !!}
</div>
    ...
    @foreach (Config::get('languages') as $lang => $language)
        ...
        <div class="tab-pane @if ($lang==App::getLocale()) fade active show @endif" id="{{ $lang }}" aria-labelledby="{{ $lang }}-tab" role="tabpanel">
            ...
            <div class="mb-2">
                ...
                {!! Form::text('title[' . $lang . ']', $block->title[$lang] ?? '', ['required', 'class' => 'form-control', 'autocomplete' => 'off', 'id' => 'title_' . $lang]) !!}
            </div>


            <div class="mb-2">
                {!! Form::label('desc-' . $lang, trans('admin.body') . ' - ' . $language , ['class' => 'form-label']) !!}
                {!! Form::textarea('desc[' . $lang . ']', $block->desc[$lang] ?? '' , [ 'class' => 'textarea form-control ckeditor', 'id' => 'ckeditor', 'placeholder' => trans('admin.desc'), 'rows' => 5 ]) !!}
            </div>
        ...
        <div class="row">
            <div class="col-1">
                <a href="" data-bs-toggle="modal" data-bs-target="#image">
                    <img src="{{ url($block->image) }}" class="img-fluid rounded-lg">
                </a>

                @include('admin.parts.modal',['id' => 'image','url' => url($block->image) ])
        ...
        <div class="row">
            <div class="col-1">
                <a href="" data-bs-toggle="modal" data-bs-target="#icon">
                    <img src="{{ url($block->icon) }}" class="img-fluid rounded-lg">
                </a>

                @include('admin.parts.modal',['id' => 'icon','url' => url($block->icon) ])
...
<div class="mb-2">
    ...
    {!! Form::select('page_id', $pages->pluck('title.'.App::getLocale(),'id'), $cover->page_id ?? Request::get('page_id'), [ 'class' => 'form-control','placeholder'=>trans('admin.choose')]) !!}
</div>


<div class="mb-2">
    {!! Form::label('sort', trans('admin.sort') , ['class' => 'form-label'] ) !!}
    {!! Form::text('sort', $block->sort ?? 1, ['required', 'class' => 'form-control']) !!}
</div>
...
<div class="mb-2">
    ...
    {!! Form::select('is_active', ['1' => trans('admin.yes'), '0' => trans('admin.no')], $block->is_active ?? 1, ['required', 'class' => 'form-control']) !!}
</div>
